...
import pandas as pd 
data = pd.read_csv('./data/cv202f.csv')
y = data['label']
X = data.loc[:,'APAAC1':]
...
w11train=X_train
w11train.to_csv('./data/cv202ftrain.csv')

w11test = pd.DataFrame()
X_test.insert(0,'label',y_test)
w11test=X_test
w11test.to_csv('./data/cv202ftest.csv')


data = pd.read_csv('./data/cv202ftrain.csv')
y = data['label']
X = data.loc[:,'APAAC1':]

data = pd.read_csv('./data/cv202ftest.csv')
yy = data['label']
XX = data.loc[:,'APAAC1':]
print('数据加载完毕！')
